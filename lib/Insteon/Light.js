  ...
  if (level) {
    ...
    deferred.resolve(
      ...
      .then(function (status) {
        ...
        var rate = Math.ceil(parseInt(status.response.extended.userData[7], 16) * 100 / 255);
        return rate;
      })
    );
    return deferred.promise.nodeify(next);
...
Light.prototype.handleCommand = function (cmd) {
  ...
  if(cmd.standard.messageType === 1 && this.emitOnAck) {
    return this.handleAck(cmd.standard.command1, cmd.standard.command2);
  }
