...
Light.prototype.handleCommand = function (cmd) {
  if(!cmd.standard || cmd.standard.id.toUpperCase() !== this.id.toUpperCase()) {
    debug('Ignoring - command not from this device (%s) : ', this.id, cmd);
    return;
  }

  if(cmd.standard.gatewayId.indexOf('0000') !== 0){
    debug('Ignoring - command not a all-link cmd device (%s) : ', this.id, cmd);
    return;
  }
  var group = parseInt(cmd.standard.gatewayId, 16);

  debug('Emitting command for device (%s)', this.id, cmd);
  this.emit('command', group, cmd.standard.command1, cmd.standard.command2);

  switch (cmd.standard.command1) {
  case '11':
  ...
  case '18':
    ...
    break;
  default:
    debug('Unreconized command - %s', cmd.standard.command1);
  }
};
