...
Thermostat.prototype.mode = function (mode, next) {
  ...
  if(mode) {
    ...
    deferred.resolve(
      this.insteon.directCommand(this.id, cmd)
      .then(function(status) {
        var mode = Object.keys(MODE_CMDS).find(function(value) { return MODE_CMDS[value] === status.response.standard.command2; });
...
Thermostat.prototype.monitor = function (enable, next) {
  ...
  if(enable) {
    ...
    return insteon.scene(id, 'gw', {group: 0xEF}, function() {
      var cmdDataSet08 = {
        cmd1: '2E',
        cmd2: '00',
        extended: true,
