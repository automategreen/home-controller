  ...
  function status() {
    deferred.resolve(
      ...
      .then(function (status) {
        ...
        details.duskThreshold = Math.ceil(parseInt(data[4], 16) * 100 / 255);

        var optionFlags = parseInt(data[5]);
        details.options = {
          occupancyMode: !(optionFlags & 0x10),
          ledOn: !!(optionFlags & 0x08),
          nightMode: !(optionFlags & 0x04),
          onMode: !(optionFlags & 0x02)
        };

        var jumperFlags = parseInt(data[8]);
        details.jumpers = {
          j1: !!(jumperFlags & 0x10),
          j2: !!(jumperFlags & 0x08),
          j3: !!(jumperFlags & 0x04),
        ...
        details.lightLevel = Math.ceil(parseInt(data[10]) * 100 / 255);
        details.batteryLevel = parseInt(data[11]) / 10;
...
Motion.prototype.setWaitCmd = function (wait, cmd, next) {
...
Motion.prototype.options = function (options, wait, next) {
  ...
  var optionBytes = toByte(
    options.occupancyMode ? 0 : 0x10 |
    options.ledOn ? 0x08 : 0 |
    options.nightMode ? 0 : 0x04 |
    options.onMode ? 0 : 0x02
  );
  ...
  var cmd = {
    ...
    cmd2: '00',
    extended: true,
    userData: ['00', '05', optionBytes]
  };

  return this.setWaitCmd(wait, cmd, next);
...
Motion.prototype.duskThreshold = function(threshold, wait, next) {
  ...
  var cmd = {
    ...
    cmd2: '00',
    extended: true,
    userData: ['00', '04', threshold]
  };

  return this.setWaitCmd(wait, cmd, next);
};

Motion.prototype.handleCommand = function (cmd) {
...
Motion.prototype.handleAllLinkBroadcast = function (group, cmd1, cmd2) {
  ...
  case 3:
    ...
    break;
  case 4:
    this.emit('heartbeat'); // not sure if this is supported
    break;
  default:
